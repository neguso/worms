...
namespace Game
{
    ...
    public List<ConsoleKey> ReadKeyPress()
		{
    ...
    public List<ConsoleKey> ReadKeyDown()
      ...
      GetKeyState(1); // needed for the following method to work
      GetKeyboardState(keys);
      return keys
        .Where(k => Enum.IsDefined(typeof(ConsoleKey), k) && (k & 0x80) != 0)
        .Select((k, i) => (ConsoleKey)i)
        .ToList();
    }
  ...
  //  public VirtualKey[] GetStatus()
		//{
  //    byte[] keys = new byte[256];
  //    GetKeyState(VirtualKey.X);
  //    GetKeyboardState(keys);
  //    return keys.Where(k => (k & 0x80) != 0).Select((k, i) => (VirtualKey)i).ToArray();
  //  }
